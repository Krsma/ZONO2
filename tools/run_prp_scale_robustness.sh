...
COMMAND="${1:?usage: tools/run_prp_scale_robustness.sh COMMAND [--smoke]}"
shift
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/pzr-prp-scale-robustness-matplotlib}"
...
cd "$ROOT_DIR"
exec "$PYTHON_BIN" tools/explore_prp_scale_robustness.py "$COMMAND" "$@"
