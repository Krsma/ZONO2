...
CONFIG="${PZR_PAPER_V4_CONFIG:-$ROOT_DIR/experiments/paper_evaluation_v4.yaml}"
COMMAND="${1:?usage: tools/run_paper_evaluation_v4.sh COMMAND [OPTIONS]}"
shift
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/pzr-paper-v4-matplotlib}"
...
cd "$ROOT_DIR"
exec "$PYTHON_BIN" tools/paper_evaluation_v4.py "$COMMAND" --config "$CONFIG" "$@"
