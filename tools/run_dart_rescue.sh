...
CONFIG="${PZR_DART_RESCUE_CONFIG:-$ROOT_DIR/experiments/dart_rescue_v1.yaml}"
COMMAND="${1:?usage: tools/run_dart_rescue.sh COMMAND [--smoke]}"
shift
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/pzr-dart-rescue-matplotlib}"
...
cd "$ROOT_DIR"
exec "$PYTHON_BIN" tools/dart_rescue_experiment.py "$COMMAND" --config "$CONFIG" "$@"
