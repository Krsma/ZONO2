...
COMMAND="${1:?usage: tools/run_clean_scaling.sh COMMAND [--smoke]}"
shift
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/pzr-clean-scaling-matplotlib}"
...
cd "$ROOT_DIR"
exec "$PYTHON_BIN" tools/explore_clean_scaling.py "$COMMAND" "$@"
