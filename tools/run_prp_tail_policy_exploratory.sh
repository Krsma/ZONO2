...
COMMAND="${1:?usage: tools/run_prp_tail_policy_exploratory.sh COMMAND [OPTIONS]}"
shift
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/pzr-prp-tail-policy-matplotlib}"
...
cd "$ROOT_DIR"
exec "$PYTHON_BIN" tools/prp_tail_policy_exploratory.py "$COMMAND" "$@"
