...
COMMAND="${1:?usage: tools/run_prp_tail_vote_guard_exploratory.sh COMMAND [OPTIONS]}"
shift
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/pzr-prp-tail-vote-guard-matplotlib}"
...
cd "$ROOT_DIR"
exec "$PYTHON_BIN" tools/prp_tail_vote_guard_exploratory.py "$COMMAND" "$@"
